...
--- 
-- Get the current mouse position in global coordinates (raw values)
    ...
    elseif ffi.os == "Linux" then
        if x11_lib ~= nil and x11_display ~= nil and x11_root ~= nil and x11_mouse ~= nil then
            if x11_lib.XQueryPointer(x11_display, x11_root, x11_mouse.root_win, x11_mouse.child_win, 
                                      x11_mouse.root_x, x11_mouse.root_y, x11_mouse.win_x, x11_mouse.win_y, 
...
--- 
-- Get the target position for zooming, ensuring the crop is centered on the mouse.
---@param zoom any
---@return table
function get_target_position(zoom)
    ...
    if monitor_info then
        -- Adjust the x coordinate by subtracting the captured display’s top‐left x.
        mouse.x = mouse.x - monitor_info.x
        
        mouse.y = mouse.y + 100 -- zakir added this to debug our issue.

        -- For macOS, flip the y coordinate using the display’s height.
        if ffi.os == "OSX" then
            mouse.y = monitor_info.display_height - (mouse.y - monitor_info.y)
        else
            mouse.y = mouse.y - monitor_info.y
        end
    end

    -- Further offset by any crop filter already applied.
    mouse.x = mouse.x - zoom.source_crop_filter.x
    mouse.y = mouse.y - zoom.source_crop_filter.y

    -- Adjust for source scaling (e.g. when using cloned scenes)
    ...
    -- Compute the new size of the crop (zoomed in view)
    local new_size = {
        width  = zoom.source_size.width / zoom.zoom_to,
        height = zoom.source_size.height / zoom.zoom_to
    }

    -- Compute the crop’s top‐left so that the new view is centered on the mouse.
    local pos = {
...
function on_timer()
    if crop_filter_info ~= nil and zoom_target ~= nil then
        ...
        if zoom_state == ZoomState.ZoomingOut or zoom_state == ZoomState.ZoomingIn then
            -- During zoom in/out animation, interpolate the crop region.
            if zoom_time <= 1 then
            ...
            -- When zoomed in and following the mouse, always update the crop to exactly center the mouse.
            if is_following_mouse then
                zoom_target = get_target_position(zoom_info)
...
function script_defaults(settings)
    ...
    -- For Retina Macs, it helps to use manual monitor override settings.
    -- Set these to match your desired “canvas” resolution (1920x1080)
    -- and apply a scaling of 2.0 to convert from native (retina) pixels.
